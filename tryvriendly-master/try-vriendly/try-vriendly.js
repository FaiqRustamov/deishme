...
//-----------session select
// constants
const sessionSelectOptions = document.querySelector('.session-select-options'),
    sessionOptions = [...sessionSelectOptions.children],
    sessionValue = document.querySelector('.session-value'),
    formSessionSelect = document.querySelector('.form-session-select'),
    sessionArrow = document.querySelector('.session-arrow-icon'),
    otherSessionInput = document.querySelector('.other-session-type');


// make visible or hidden session select options
formSessionSelect.addEventListener('click', (e) => {
    if (sessionArrow.classList[1] === "active") {
        hideSessionOptions();
    } else {
        hideDeviceOptions();
        showSessionOptions();
...
function hideSessionOptions() {
    sessionArrow.classList.remove('active');
    sessionSelectOptions.style.visibility = "hidden";
    sessionSelectOptions.style.opacity = 0;
}

function showSessionOptions() {
    sessionArrow.classList.add('active');
    sessionSelectOptions.style.visibility = "visible";
    sessionSelectOptions.style.opacity = 1;
}
...
function removeSessionActive() {
    for (let option in sessionOptions) {
        if (sessionOptions[option].innerHTML !== "Other*") {
            // console.log(options[option].innerHTML);
            otherSessionInput.style.display = "none";
        }
        sessionOptions[option].classList.remove('active');
    }
}


// get active element value
function sessionActiveElementValue() {
    for (let option in sessionOptions) {
        if (sessionOptions[option].classList[1] === "active") {
            if (sessionOptions[option].innerHTML === "Other*") {
                // console.log(options[option].innerHTML);
                otherSessionInput.style.display = "block";
            }
            return sessionOptions[option].innerHTML;
        }
    }
}

// chose session select option
sessionSelectOptions.addEventListener('click', (e) => {
    const selectedOption = e.target;
    removeSessionActive();
    selectedOption.classList.add('active');
    sessionValue.innerHTML = sessionActiveElementValue()
});
...
function hideDeviceOptions() {
    ...
    sessionArrow.style.display = "block";
    formSessionSelect.style.position = "relative"
}
...
function showDeviceOptions() {
    ...
    sessionArrow.style.display = "none";
    formSessionSelect.style.position = "static";
}
...
function resetForm() {
    registerForm.reset();
    sessionValue.innerHTML = "Session type";
...
registerForm.addEventListener('submit', (e) => {
    ...
    for (let element = 0; element < 11; element = element + 1) {


        if (((e.target.children[element].value || e.target.children[element].innerText) !== "") || ((e.target.children[element].innerText) === "Device") || ((e.target.children[element].innerText) === "Session type")) {
    ...
    function userIsFull() {
        for (let key of userArray) {
            if (user[key] === "" || user[key] === "Device" || user[key] === "Session type") {
                return
            }
        }
        return true
