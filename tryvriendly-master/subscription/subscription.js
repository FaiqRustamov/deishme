...
const menuHeadingPricing = document.querySelector("body > header > nav.big-screen > ul > li.pricing > p");
...
const pricing = document.querySelector("body > header > nav.big-screen > ul > li.pricing > div.items");
...
const pricingItems = document.querySelectorAll("body > header > nav.big-screen > ul > li.pricing > div > a");
...
menuHeadingPricing.addEventListener('mouseenter', openPricing);
...
pricing.addEventListener('mouseleave', closePricing);
...
function openPricing() {
    ...
    closeWhatWeDo();
    closeOrderVriendly();
    let el = menuHeadingPricing.childNodes[0];
    el.classList.add('three-d-rotate');
    setTimeout(() => {
        pricingAreItems.forEach(item => {
            item.classList.add('fade-down');
        });
    }, 300);
}
